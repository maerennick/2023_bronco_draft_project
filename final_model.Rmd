...
## A Mean Squared Error (MSE) value of 0.068 is quite low, which suggests that the model's predictions are generally close to the actual values in the test dataset
...
If shuttle speed is the most important? What is the ranking of shuttle 
...
selected_columns <- c('ht', 'bench', 'vertical', 'forty', 'broad_jump', 'cone', 'wt', 'shuttle')
...
current_broncos_clusters<- data_with_clusters %>% 
  ...
  group_by(pfr_id) %>% 
  summarise(mean(ht),mean(season.x), mean(wt),mean(forty),mean(bench),mean(vertical),mean(broad_jump),mean(cone),mean(shuttle), mean(cluster))
